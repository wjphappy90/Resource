									...
									<TD>
										<TABLE id=grid style="BORDER-TOP-WIDTH: 0px; FONT-WEIGHT: normal; BORDER-LEFT-WIDTH: 0px; BORDER-LEFT-COLOR: #cccccc; BORDER-BOTTOM-WIDTH: 0px; BORDER-BOTTOM-COLOR: #cccccc; WIDTH: 100%; BORDER-TOP-COLOR: #cccccc; FONT-STYLE: normal; BACKGROUND-COLOR: #cccccc; BORDER-RIGHT-WIDTH: 0px; TEXT-DECORATION: none; BORDER-RIGHT-COLOR: #cccccc" cellSpacing=1 cellPadding=2 rules=all border=0>
											<TBODY>
												...
												<%--s:iterator标签：用于在页面上遍历集合
														value：指定遍历的集合。它的取值是一个OGNL表达式
														var：使用了该属性：属性的取值是一个普通的字符串。当执行遍历时，会把var属性的值当成key，把当前遍历的对象当成value。把他们存入ContextMap中
															 没使用该属性：把当前遍历的对象压入栈顶。遍历结束后再弹栈。
														begin：指定遍历开始的索引
														end:指定遍历结束的索引
														step：指定步长。
														status：遍历的计数器对象
												...
												<TR ${vs.odd?"style='background-color:#D1EEEE'":"style='background-color:#FFEC8B'" } style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
													<TD>${custName }</TD>
													<TD>${custLevel }</TD>
													<TD>${custSource }</TD>
													<TD>${custIndustry }</TD>
													...
													<TD>
													<a href="${pageContext.request.contextPath }/customer/CustomerServlet?method=editCustomerUI&custId=<s:property value="#cust.custId"/>">修改</a>
													&nbsp;&nbsp;
													<a href="${pageContext.request.contextPath }/customer/CustomerServlet?method=removeCustomer&custId=<s:property value="#cust.custId"/>">删除</a>
													</TD>--%>
												</TR>	
												</s:iterator>
											</TBODY>
